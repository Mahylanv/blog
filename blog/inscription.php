...
include 'inc/fonctions.inc.php'; // des fonctions utiles
    ...
    if( isset($_POST['pseudo']) && isset($_POST['email']) && isset($_POST['mdp']) ) {
        ...
        if($taille_pseudo < 4 || $taille_pseudo > 16) {
            $msg .= '<div class="alert alert-danger mb-3">Attention : le pseudo doit avoir entre 4 et 16 caractères</div>';
            $erreur = true;
        } 
        ...
        */
        if($verif_caracteres != true) {
            $msg .= '<div class="alert alert-danger mb-3">Attention : le pseudo ne peut contenir ques des caracteres alphanumériques</div>';
            $erreur = true;
        }
        ...
        if($verif_pseudo->rowCount() > 0) /* si on a plus de 0 lignes qui correspondent*/ {
            $msg .= '<div class="alert alert-danger mb-3">Attention : le pseudo est indisponible</div>';
            $erreur = true;
        }
        ...
        if( ! filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $msg .= '<div class="alert alert-danger mb-3">Attention : le format du mail ne correspond pas</div>';
            $erreur = true;
        }
        ...
        if($verif_mail->rowCount() > 0) /* si on a plus de 0 lignes qui correspondent*/ {
            $msg .= '<div class="alert alert-danger mb-3">Attention : le mail est indisponible</div>';
            $erreur = true;
        }

        // taille du mdp : minimum 4
        if(iconv_strlen($mdp) < 4) {
            $msg .= '<div class="alert alert-danger mb-3">Attention : le mot de passe doit avoir 4 caracteres minimum</div>';
            $erreur = true;
        } 
...
?>
        ...
        <div class="bg-light p-5 rounded">
            <h1 class="text-center border-bottom pb-3">Inscription</h1>
            <p class="lead text-center">Inscrivez-vous !</p>
            <?php echo $msg; ?>
        </div>
